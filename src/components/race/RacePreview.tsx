...
const RacePreview: React.FC = () => {
    ...
    return (
        <IonList lines="full">
            ...
            <IonItem button
                     routerLink={`/race/${race.season}/${race.round}/${race.Circuit.Location.country}/${race.Circuit.circuitName}`}>
                <div slot="start" className="race-date ion-text-center">
                    <strong>{new Date(race.date).getDate()}</strong><br/>
                    <IonBadge color="medium"
                              mode="ios">{new Date(race.date).toLocaleString('default', {month: 'short'})}</IonBadge>
                </div>
                <IonLabel>
