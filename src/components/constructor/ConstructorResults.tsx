...
const ConstructorResults: React.FC<{ season: string, constructorId: string }> = ({season, constructorId}) => {
    ...
    return (
        <IonList lines="full" className="constructor-results-list">
            ...
            {results.map(result =>
                <ConstructorResult season={season} round={result.round} constructorId={constructorId}
                                   country={result.Circuit.Location.country} date={result.date} key={result.raceName}/>
            )}
