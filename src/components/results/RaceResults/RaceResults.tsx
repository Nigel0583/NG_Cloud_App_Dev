        ...
        <IonList lines="full">
            ...
            {results[0].Results.map(result =>
                <IonItem key={result.position} className={result.FastestLap?.rank === '1' ? 'fastest-lap' : undefined}>
                    ...
                    <IonLabel>
                        <h3>{result.Driver.givenName} <strong
                            className="ion-text-uppercase">{result.Driver.familyName}</strong></h3>
                        <p>{result.Constructor.name}</p>
                    </IonLabel>
                    <IonBadge color="medium" mode="ios"
                              className="race-time">{result.Time ? result.Time.time : (result.status === '+1 Lap' ? result.status : 'DNF')}</IonBadge>
