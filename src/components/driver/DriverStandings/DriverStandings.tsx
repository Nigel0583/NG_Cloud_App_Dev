        ...
        <IonList lines="full">
            {drivers.map(driver =>
                <IonItem button onClick={() => _handleClick(driver.Driver.driverId)} key={driver.Driver.driverId}>
                    ...
                    <IonIcon lazy className="constructor-logo ion-margin-end"
                             src={`https://bucketofflags103632-staging.s3.amazonaws.com/constructors/${driver.Constructors[0].constructorId}.svg`}/>
                    <IonLabel>
                        <h3>{driver.Driver.givenName} <strong
                            className="ion-text-uppercase">{driver.Driver.familyName}</strong></h3>
                        <p>{driver.Constructors[0].name}</p>
                    </IonLabel>
                    <IonBadge className="standings-points" slot="end" color="medium"
                              mode="ios">{driver.points}</IonBadge>
                </IonItem>
            )}
        </IonList>
    );
